...
import { apMicro, getUnit, getTopic, getNotes, getQuestions } from "@/lib/content";
...
export default function NotesPage({
  ...
  const maybeNotes = getNotes(unitSlug, topicSlug);
  if (!maybeUnit || !maybeTopic || !maybeNotes) notFound();
  const unit = maybeUnit;
  const topic = maybeTopic;
  const notes = maybeNotes;
  const hasPractice = !!getQuestions(unitSlug, topicSlug);
