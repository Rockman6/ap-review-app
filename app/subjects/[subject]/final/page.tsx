...
async function saveCompletion(args: { score: number; subjectSlug: string }) {
  const supabase = createClient();
  const { data } = await supabase.auth.getUser();
  if (!data.user) return;
...
export default function FinalPracticePage({
  ...
  const maybeQuestions = getFinalQuestions(subjectSlug);
  if (!maybeSubject || !maybeQuestions || maybeQuestions.length === 0) notFound();
  ...
  useEffect(() => {
    ...
    void saveCompletion({ score: correctCount / total, subjectSlug: subject.slug });
  }, [finished, correctCount, total, subject.slug]);
    ...
    <div className="mx-auto max-w-2xl px-4 py-10 sm:py-14">
      ...
      <div className="mt-4 flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-widest text-amber-600">
          {t({ en: `Final · ${total} questions`, zh: `综合 · ${total} 题` })}
        </p>
        {!finished && (
  ...
  return (
    <div className="py-4 text-center">
      ...
      <h2 className="mt-4 text-2xl font-semibold text-slate-900">
        {t({ en: "Final review complete!", zh: "期末综合复习完成!" })}
      </h2>
